...
api.nvim_create_user_command('Recompile', function()
  ...
  close_running()
  -- last_cmd is already stripped, so re-deriving `silent` from it always came
  -- back false and a `++silent` build became noisy on the second run.
  compiler(last_cmd, api.nvim_buf_get_name(0), { silent = last_silent })
end, {})
...
api.nvim_create_user_command('CompileSet', function(args)
  ...
  if write_compile_command(cmd) then
    local stripped, silent = strip_silent(cmd)
    last_cmd = stripped
    last_silent = silent
  end
